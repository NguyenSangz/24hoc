...
async function startServer() {
  ...
  import('./services/gemini').then(gemini => {
    ...
    app.post('/api/gemini/maze-questions', async (req, res) => {
      try {
        const { grade, subject, difficulty, count } = req.body;
        const questions = await gemini.generateMazeQuestions(grade, subject, difficulty, count);
        res.json({ questions });
      } catch (err) {
        console.error('Gemini maze questions error:', err);
        res.status(500).json({ error: 'AI error' });
      }
    });
    ...
    app.post('/api/gemini/analyze-error', async (req, res) => {
      try {
        const { questionText, options, userAnswerIndex, correctAnswerIndex, explanation } = req.body;
        const analysis = await gemini.analyzeThinkingError(questionText, options, userAnswerIndex, correctAnswerIndex, explanation);
        res.json({ analysis });
      } catch (err) {
        console.error('Gemini analyze-error error:', err);
        res.status(500).json({ error: 'AI error' });
      }
    });

    app.post('/api/gemini/lesson-guide', async (req, res) => {
      try {
        const { grade, subject, semester, curriculum } = req.body;
        const result = await gemini.generateStudyGuide(grade, subject, semester, curriculum);
        if (!result) return res.status(500).json({ error: 'AI error' });
        res.json({ result });
      } catch (err) {
        console.error('Gemini lesson-guide error:', err);
        res.status(500).json({ error: 'AI error' });
      }
    });

    app.post('/api/gemini/lesson-details', async (req, res) => {
      try {
        const { grade, subject, lessonTitle, curriculum } = req.body;
        const result = await gemini.generateLessonDetails(grade, subject, lessonTitle, curriculum);
        if (!result) return res.status(500).json({ error: 'AI error' });
        res.json({ result });
      } catch (err) {
        console.error('Gemini lesson-details error:', err);
        res.status(500).json({ error: 'AI error' });
      }
    });
    ...
    app.post('/api/gemini/mindmap', async (req, res) => {
      try {
        const { grade, subject, lessonTitle, curriculum } = req.body;
        const result = await gemini.generateMindMap(grade, subject, lessonTitle, curriculum);
        if (!result) return res.status(500).json({ error: 'AI error' });
        res.json({ result });
      } catch (err) {
        console.error('Gemini mindmap error:', err);
        res.status(500).json({ error: 'AI error' });
      }
    });
    ...
    app.post('/api/gemini/chat', async (req, res) => {
      try {
        ...
        const text = await gemini.sendChatMessage(messages, lessonTitle, lessonContent);
        res.json({ text });
      } catch (err) {
    ...
    app.post('/api/gemini/speech', async (req, res) => {
      try {
        ...
        if (!audioBase64) return res.status(500).json({ error: 'AI error' });
